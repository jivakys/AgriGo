...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function startCountdown() {
    ...
    timerText.innerHTML =
      'Resend code in <span id="countdown">' + timeLeft + "</span> sec";
    timerId = setInterval(() => {
      timeLeft--;
  ...
  verifyOtpBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:3000/auth/user/verify-reset-otp",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  resetPasswordForm.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          data.error &&
          (data.error.includes("Invalid") || data.error.includes("expired"))
        ) {
      ...
      submitBtn.innerHTML =
        '<i class="fas fa-check me-2"></i>Create new password';
    }
  });
